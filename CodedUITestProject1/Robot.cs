...
namespace CodedUITestProject1
{
    public class Robot
    {
        ...
        {
            //Console.Write(_root.Name);
            WinButton button = (WinButton)Robot.FindWinControl(typeof(WinButton), name, _root);
            Mouse.Click(button);
        }        

        public static void FindWindow(string formName)
        {
            _root = Robot.FindWinControl(typeof(WinWindow), formName, null) as WinWindow;
        ...
        public static void SetComboBox(string name, string targetName, string formName)
        {
            WinWindow sec = Robot.FindWinControl(typeof(WinWindow), formName, null) as WinWindow;
            WinComboBox comboBox = (WinComboBox)Robot.FindWinControl(typeof(WinComboBox), name, sec);
            if (comboBox.SelectedItem != targetName)
                comboBox.SelectedItem = targetName;
        }
        ...
        public static void ClickNumericUpDownButtonInDataGridView(string dataGridViewName, int rowIndex, int columnIndex, NumericDirect direct, int times)
        {
            ...
            System.Drawing.Rectangle boundingRectangle = cell.BoundingRectangle;
            ...
            switch (direct)
            {
                case NumericDirect.UP:
                    ...
                    Mouse.Click(new System.Drawing.Point(boundingRectangle.X + boundingRectangle.Width - HALF_BUTTON_WIDTH, boundingRectangle.Y + upperPartYOffset));
                    break;
                case NumericDirect.DOWN:
                    Mouse.Click(new System.Drawing.Point(boundingRectangle.X + boundingRectangle.Width - HALF_BUTTON_WIDTH, boundingRectangle.Y + lowerPartYOffset));
                    break;
            }
        ...
        public static void ClickDeleteButtonInDataGridView(string dataGridViewName, int rowIndex)
        {
            int columnIndex = 0;
            const int HALF_BUTTON_WIDTH = 8;
            const int SPINBUTTON_HEIGHT_FINE_TUNE = 3;
        ...
        public static void ClickDataListItem(string name, string index)
        {            
            WinList listbox = Robot.FindWinControl(typeof(WinList), name, _root) as WinList;
            WinListItem item = listbox.Items[int.Parse(index)] as WinListItem;
            Mouse.Click(item);
        }

        public static void AssertDataListItem(string name, int value)
        {
            WinList listbox = Robot.FindWinControl(typeof(WinList), name, _root) as WinList;
            Assert.AreEqual(listbox.Items.Count, value);
        }
    }
}
